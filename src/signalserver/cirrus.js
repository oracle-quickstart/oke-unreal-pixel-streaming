...
if (config.UseHTTPS) {
  ...
  const certDir = path.resolve(config.certificateDir);
  console.log(`SSL certificate directory: ${certDir}`);
	const options = {
...
if (config.UseHTTPS) {
	...
	app.use(function (req, res, next) {
		...
		res.setHeader('Content-Security-Policy', "default-src 'self' *; frame-ancestors 'self' *; style-src 'self' 'unsafe-inline' 'unsafe-eval' https://stackpath.bootstrapcdn.com  https://use.fontawesome.com; script-src 'self' 'unsafe-inline' 'unsafe-eval' https://stackpath.bootstrapcdn.com https://code.jquery.com https://cdnjs.cloudflare.com https://webrtc.github.io");
		next();
	});
}
...
streamerServer.on('connection', function (ws, req) {
  ...
  // MV: use the client config provider instead
...
playerServer.on('connection', function (ws, req) {
	...
	ws.on('message', function (msg) {
      ...
      // MV Custom: don't close just because a message noops
